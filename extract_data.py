...
print("==================================================================")
print("   BẮT ĐẦU QUÁ TRÌNH TRÍCH XUẤT DỮ LIỆU BIDV (2014 - NAY) - VCI")
print("==================================================================")

# ==============================================================================
# HỢP PHẦN 1: TẢI VÀ GỘP LỊCH SỬ GIÁ CỔ PHIẾU BID
# ==============================================================================
def extract_stock_history():
    print("\n[1/3] Đang tải lịch sử giá cổ phiếu BID...")
    m = Market()
    
    # Lấy phân đoạn 1: 2014-01-01 đến 2020-12-31
    print("      -> Đang tải Phân đoạn 1 (2014 - 2020)...")
    try:
        df1 = m.equity('BID').ohlcv(start='2014-01-01', end='2020-12-31', resolution='1D', count=5000, source='vci')
        print(f"         Thành công. Số phiên: {df1.shape[0]}")
    except Exception as e:
        print(f"         Thất bại khi tải Phân đoạn 1: {e}")
        df1 = pd.DataFrame()

    # Lấy phân đoạn 2: 2021-01-01 đến 2026-06-21
    print("      -> Đang tải Phân đoạn 2 (2021 - Nay)...")
    try:
        df2 = m.equity('BID').ohlcv(start='2021-01-01', end='2026-06-21', resolution='1D', count=5000, source='vci')
    ...
    if not df1.empty or not df2.empty:
        ...
        output_path = os.path.join(DATA_DIR, 'bid_stock_history.csv')
        ...
        print("      [ERROR] Không tải được dữ liệu lịch sử giá cổ phiếu.")
...
def main():
    ...
    extract_stock_history()
    
    # 2. Tải Báo cáo tài chính (CDKT, KQKD, LCTT) từ VCI
    f = VCIFinance(symbol='BID', show_log=False)
    
    reports = {
        'balance_sheet': 'bid_balance_sheet',
        'income_statement': 'bid_income_statement',
        'cash_flow': 'bid_cash_flow'
    }
    
    periods = ['annual', 'quarterly']
    all_mappings = []
    
    print("\n[2/3] Đang trích xuất và xoay trục báo cáo tài chính BIDV từ VCI...")
    
    for rkey, file_prefix in reports.items():
        for p in periods:
            ...
            try:
                ...
                if not df_ts.empty:
                    output_file = os.path.join(DATA_DIR, f"{file_prefix}_{p}.csv")
    ...
    print("\n      -> Đang tải FINANCIAL RATIOS (ANNUAL & QUARTERLY)...")
    try:
        ...
        ratio_ann_file = os.path.join(DATA_DIR, 'bid_financial_ratios_annual.csv')
        ...
        ratio_qtr_file = os.path.join(DATA_DIR, 'bid_financial_ratios_quarterly.csv')
        ...
        for fid, vname in RATIO_COLUMN_MAP_VI.items():
            ...
            ratio_mapping_rows.append({
                'item': vname,
                'item_en': enname if 'enname' in locals() else ensub if 'ensub' in locals() else ename,
                'item_id': fid,
                'report_type': 'ratios'
            })
        ...
        print(f"         [ERROR] Thất bại khi tải Chỉ số tài chính: {e}")

    # 4. Tạo file Metadata mapping từ điển chỉ tiêu tài chính
    print("\n[3/3] Đang tạo từ điển dữ liệu chỉ tiêu tài chính...")
    if all_mappings:
