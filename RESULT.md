...
| **3. Tinh gọn Star Schema** | Hợp nhất 4 bảng Fact chứng khoán cũ thành 1 bảng duy nhất: `fact_stock_daily_metrics`. Star Schema rút gọn xuống còn **7 bảng kho dữ liệu lõi** (5 Dimensions & 2 Facts) cùng **3 bảng kết quả mô hình học máy** (tổng cộng 10 bảng BigQuery). | Tối ưu hóa hiệu năng lưu trữ cột (Columnar Storage) của BigQuery, tăng tốc các truy vấn JOIN và giảm chi phí quét dữ liệu trên đám mây. |
