...
graph TD
    ...
    A1 -->|Crawl dữ liệu| B1
    B1 -->|Xuất giá và khối lượng| B5
    B5 -->|Tính chỉ số & Xuất CSV hợp nhất| CSV_Fact_Cons
    CSV_Fact_Cons -->|Nạp dữ liệu MERGE| D_Fact_Stock
    
    %% Kết nối luồng Ngân hàng (Bank Flow)
    A2 -->|Đọc file BCTC| B1
    B1 -->|Xoay trục BCTC| B4
    ...
    %% Kết nối Kiểm toán
    B2 & B4 & B5 -->|Ghi nhật ký chạy| D_Audit
...
1. **Trích xuất (Extract):** Tệp `src/etl/extract_data.py` gọi API `vnstock` để tải dữ liệu lịch sử giá của 4 mã ngân hàng (BID, TCB, VCB, CTG) từ ngày 01-01-2014 đến hiện tại. Kết quả được lưu cục bộ dưới dạng các tệp CSV tại thư mục `data/raw/` hoặc `data/processed/{symbol}/`.
2. **Biến đổi & Hợp nhất (Transform & Consolidate):** Tệp `src/etl/consolidate_stock_metrics.py` đọc dữ liệu thô, loại bỏ các dòng bị khuyết thiếu giá đóng cửa `close_price`. Sau đó, thực hiện chuyển đổi định dạng ngày tháng thành khóa thay thế `date_key` (INT64 dạng `YYYYMMDD`), gán `stock_key` (BID = 1, TCB = 2, VCB = 3, CTG = 4), tính toán 5 chỉ số động lực học phái sinh và gán các trường kiểm toán (`_created_at`, `_updated_at`, `_source_file`), rồi xuất ra tệp `fact_stock_daily_metrics_clean.csv`.
3. **Nạp Kho (Load to DWH):** Tệp `src/etl/load_to_bigquery.py` đẩy dữ liệu từ CSV sạch lên bảng đích `fact_stock_daily_metrics` của BigQuery thông qua API Load Job bằng phương pháp `MERGE` (đảm bảo tính tuần tự và không trùng lặp).
...
1. **Trích xuất (Extract):** Tệp `src/etl/extract_data.py` trích xuất bảng cân đối kế toán và báo cáo kết quả hoạt động kinh doanh từ tệp Excel thô `data/raw/VN banks dataset (updated August 2023).xlsx`, thực hiện xoay trục dữ liệu để chuyển từ định dạng cột báo cáo năm thành định dạng chuỗi dọc và xuất ra tệp CSV trung gian.
