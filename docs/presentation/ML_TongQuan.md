...
|---------|---------|---------------------|---------------------|
| **LSTM** | Học có giám sát — Hồi quy chuỗi thời gian | Dự báo giá đóng cửa cổ phiếu T+1 đến T+5 | Q1: Dòng tiền ngoại và tự doanh ảnh hưởng thế nào đến giá BID? Q2: Biến động giá 4 cổ phiếu đồng pha hay phân hóa? |
| **K-Means + PCA** | Học không giám sát — Phân cụm | Phân nhóm 46 ngân hàng thương mại theo chiến lược hoạt động | Q4: Có phân loại rõ rệt chiến lược hoạt động ngân hàng không? |
...
- LSTM cho phép đưa vào **nhiều biến đầu vào cùng lúc** — không chỉ giá lịch sử mà còn khối lượng giao dịch, dòng tiền ngoại, dòng tiền tự doanh — để mô hình học được mối liên hệ đa chiều.
...
LSTM của đồ án sử dụng **hai bộ biến khác nhau** tùy theo mã cổ phiếu:

#### BID — 12 features (dữ liệu phong phú nhất)
...
| `foreign_net_volume` | Khối lượng mua/bán ròng của khối ngoại | Tín hiệu "smart money" — nhà đầu tư nước ngoài có thông tin sớm hơn nhà đầu tư cá nhân |
| `foreign_net_value` | Giá trị mua/bán ròng của khối ngoại (VND) | Lượng hóa tác động thực tế bằng tiền, không chỉ khối lượng |
| `prop_net_volume` | Khối lượng mua/bán ròng của tự doanh | Tín hiệu từ chính bộ phận tự doanh của các công ty chứng khoán |
| `prop_net_value` | Giá trị mua/bán ròng của tự doanh (VND) | Lượng hóa tác động vốn từ tự doanh |
...
| `foreign_net_lag_1` | Khối lượng ròng khối ngoại trễ 1 phiên | Giả thuyết: mua ròng phiên hôm qua ảnh hưởng đến giá hôm nay |
| `prop_net_lag_1` | Khối lượng ròng tự doanh trễ 1 phiên | Tương tự, tín hiệu tự doanh có độ trễ 1 phiên |

#### TCB, VCB, CTG — 7 features (chỉ dữ liệu giá)

| Biến | Ý nghĩa | Tại sao chọn |
|------|----------|--------------|
| `close_price` | Giá đóng cửa phiên giao dịch | Biến mục tiêu |
| `open_price` | Giá mở cửa | Phản ánh kỳ vọng đầu phiên |
| `high_price` | Giá cao nhất | Ngưỡng kháng cự |
| `low_price` | Giá thấp nhất | Ngưỡng hỗ trợ |
| `trading_volume` | Khối lượng giao dịch | Thước đo thanh khoản |
| `price_change_pct` | Phần trăm thay đổi giá đóng cửa | Tín hiệu momentum |
| `volume_change_pct` | Phần trăm thay đổi khối lượng giao dịch | Phát hiện thanh khoản đột biến, thường đi trước các sóng giá lớn |

**Tại sao BID có nhiều biến hơn?** Vì dự án thu thập được dữ liệu giao dịch khối ngoại và tự doanh riêng cho BID. Ba mã còn lại chỉ có dữ liệu giá lịch sử trong kho dữ liệu, nên sử dụng biến phái sinh từ chính dữ liệu giá để bổ sung thông tin.
...
|------------------|---------|----------|
| **RMSE — Root Mean Squared Error** | BID: **0.9167**, TCB: **1.3725**, VCB: **2.9453**, CTG: **1.5025** | Đơn vị tính giống đơn vị giá (nghìn VND). RMSE 0.9167 nghĩa là trung bình mỗi dự báo lệch khoảng 917 VND so với giá thực tế. RMSE càng thấp càng tốt. |
...
| **Mức cải thiện** | BID: giảm 21.6% sai số, TCB: giảm 85.5%, VCB: giảm 34.4%, CTG: giảm 86.8% | LSTM vượt trội hoàn toàn so với ARIMA trên cả 4 mã. Đặc biệt, TCB và CTG cải thiện trên 85%. |
...
- Với BID, việc tích hợp dòng tiền ngoại và tự doanh làm đầu vào giúp giảm sai số từ 1.1696 xuống 0.9167, xác nhận dòng tiền "smart money" là tín hiệu dẫn dắt giá ngắn hạn.
...
- 46 ngân hàng được mô tả bởi 47+ chỉ số tài chính — quá nhiều chiều để trực quan hóa hoặc phân tích thủ công. PCA giảm chiều giúp nhìn thấy cấu trúc dữ liệu trên không gian 2D.
...
|---------------------|------------|----------------|
| **Q1:** Dòng tiền ngoại/tự doanh ảnh hưởng thế nào đến giá BID? | LSTM | Có ảnh hưởng tích cực — LSTM với 12 features đạt RMSE 0.9167, vượt trội hơn ARIMA (1.1696). Lực mua ròng ngoại thường đi trước biến động tăng giá 1-2 phiên. |
