...
## 5. Cập nhật mới nhất cho Mô hình Triển khai (Production Models & Streamlit)

Trong phiên bản cập nhật mới nhất, toàn bộ hệ thống huấn luyện và giao diện trực quan hóa đã được tối ưu hóa cho sản xuất:

### 5.1. Mở rộng dữ liệu kho BigQuery (DWH)
* Bảng `fact_bank_performance` đã được mở rộng thành công lên đầy đủ **31 cột chỉ số tài chính CAMELS** (thay vì rút gọn 11 cột như trước), đồng bộ hoàn toàn với dữ liệu thực tế lịch sử.

### 5.2. Xuất bản Artifacts mô hình sản xuất (`reports/models/`)
* **Mô hình LSTM đa ngân hàng**: Đã mở rộng huấn luyện cho cả 4 ngân hàng trọng tâm (`BID`, `TCB`, `VCB`, `CTG`). Mô hình được xuất dưới dạng `.keras` (`lstm_bid_price.keras`, v.v.) và các bộ chuẩn hóa MinMaxScaler tương ứng dưới dạng `.pkl`.
* **K-Means & PCA**: Lưu các mô hình phân cụm (`kmeans_model.pkl`), giảm chiều (`pca_model.pkl`) và StandardScaler (`scaler_bank.pkl`).
* **Random Forest**: Lưu mô hình phân lớp rủi ro tín dụng (`random_forest_credit_risk.pkl`), danh sách thuộc tính (`rf_features.pkl`), và ngưỡng tối ưu tìm được (`rf_threshold.pkl` = `0.2327`).

### 5.3. Streamlit Interactive Dashboard (`src/dashboard/app.py`)
Đã xây dựng ứng dụng Dashboard trực quan hóa tương tác kết nối trực tiếp với BigQuery (sử dụng phương thức bypass Storage API để tránh lỗi phân quyền HTTP 403) gồm 4 phân hệ chính:
1. **Market Price Forecasting (LSTM)**: Biểu đồ đường trực quan giá thực tế và dự báo T+1 đến T+5 cùng bảng số liệu thay đổi %.
2. **Bank Clustering (K-Means)**: Biểu đồ phân tán 2D PCA các cụm ngân hàng, biểu đồ cột so sánh hồ sơ trung bình CAMELS của các cụm và danh sách bộ lọc theo phân loại ngân hàng.
3. **Credit Risk Classifier (RF)**: Phân bổ tỷ lệ rủi ro nợ xấu, biểu đồ cột độ quan trọng của đặc trưng Random Forest, và bảng giám sát cảnh báo rủi ro trực tiếp (🚨 High Risk).
4. **DWH System Status**: Theo dõi siêu dữ liệu và số lượng dòng thực tế của 10 bảng Star Schema trong BigQuery.
