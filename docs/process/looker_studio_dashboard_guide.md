...
2. Trong tab **Thiết lập (Setup)**:
   ...
   * Trục X (Metric X): Chọn chỉ số `roa` (Đổi tên nhãn hiển thị thành `Tỷ suất sinh lời ROA`).
   * Trục Y (Metric Y): Chọn chỉ số `npl_ratio` (Đổi tên nhãn hiển thị thành `Tỷ lệ nợ xấu (NPL Ratio)`).
3. Trong tab **Kiểu (Style)**:
...
2. Trong tab **Thiết lập (Setup)**:
   ...
   * Chỉ số trục Y (Metric): Chọn trường `npl_ratio` (Định dạng hiển thị là phần trăm).
